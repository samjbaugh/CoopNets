...
from model.data_io import saveSampleResults
from model.data_io_mnist import DataSetMnist


class CoopNetsAd(object):
    def __init__(self, w1_load, num_epochs=200, image_size=64, batch_size=100, nTileRow=12, nTileCol=12, net_type='object',
    ...
    def build_model(self):
        self.gen_res = self.generator(self.z, reuse=True)

        obs_res = self.descriptor(self.obs, reuse=True)
        ...
        # attractor
        w0 = tf.get_collection(tf.GraphKeys.W)
        w1 = [tf.constant(w) for w in self.w1_load]

        # distance
        self.distance = self.distance_l2(w0, w1)

        # objective
        alpha = tf.constant(10, dtype=tf.float32, name='alpha')
        attract = self.attract_l2(alpha, w0, w1)
        self.des_loss_ad = tf.add(self.des_loss, attract, name='des_loss_ad')


        des_optim = tf.train.AdamOptimizer(self.d_lr, beta1=self.beta1)
        des_grads_vars = des_optim.compute_gradients(self.des_loss_ad, var_list=des_vars)
    ...
    def attract_l2(self, alpha, w0, w1):
        l2 = tf.Variable(tf.constant(0.0), name='obj_l2')
        for i in range(len(w0)):
            norm = tf.norm(tf.subtract(tf.reshape(w0[i], [-1]), tf.reshape(w1[i], [-1])))
            norm = tf.Print(norm, [norm])
            l2 = tf.add(l2, tf.multiply(alpha, norm))
        return l2

    def distance_l2(self, w0, w1):
        l2 = tf.Variable(tf.constant(0.0), name='dist_l2')
        for i in range(len(w0)):
            norm = tf.norm(tf.subtract(tf.reshape(w0[i], [-1]), tf.reshape(w1[i], [-1])))
            l2 = tf.add(l2, norm)
        return l2
    ...
    def train(self, sess):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(num_batches):
                ...
                dist = self.distance.eval()

                sample_results[i * self.num_chain:(i + 1) * self.num_chain] = syn
                print('Epoch #{:d}, [{:2d}]/[{:2d}], descriptor loss: {:.4f}, '
                      'L2 distance: {:4.4f}, W distance {:4.4f}'.format(epoch, i+1, num_batches, d_loss, mse, dist))
            ...
            [des_loss_avg, mse_avg, summary] = sess.run([self.des_loss_mean,
                                                                       self.recon_err_mean, self.summary_op])
            end_time = time.time()
            print('Epoch #{:d}, avg. descriptor loss: {:.4f}, avg. L2 distance: {:4.4f}, '
                  'time: {:.2f}s'.format(epoch, des_loss_avg, mse_avg, end_time - start_time))
