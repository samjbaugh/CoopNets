...
from model.model_ad import CoopNetsAd
...
def main(_):
    ...
    w1_load = load_weights('./models/net-1/model')
    ...
    with tf.Session() as sess:
        model.load(sess, "./models/net-%d/model" % 0)
        model.train(sess)
...
def load_weights(file):
    with tf.Session().as_default() as session:
        ...
        w = session.run(tf.get_collection(tf.GraphKeys.W))
